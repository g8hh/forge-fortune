...
var preloader = {
    ...
    contentLoading: function contentLoading() {
        ...
        setTimeout(function() {
            if (!_this.loaded)
                preloaderExtended.classList.remove("loaded")
        }, 25e3)
    },
...
function refreshPatchNotes() {
    if (PatchManager.updateNeeded())
        $("#versionNum").addClass("hasEvent")
}
...
function openTab(tabName) {
    ...
    if (tabName === "inventoryTab") {
        ...
        if (TownManager.typeToBuilding("bank").status === BuildingState.built)
            $("#goToBank").show();
        else
            $("#goToBank").hide();
        refreshInventory()
...
$comptitle1.click(function(e) {
    e.preventDefault();
    if (!recipeList.idToItem("R13001").craftCount > 0)
        return;
...
$comptitle3.click(function(e) {
    e.preventDefault();
    if (!Shop.alreadyPurchased("AL1000"))
        return;
    tabClick(e, "guildTab")
});
$comptitle4.click(function(e) {
    e.preventDefault();
    if (!AreaManager.idToArea("A01").unlocked())
        return;
...
function generateTabVisibleTabList() {
    ...
    if (HeroManager.heroOwned("H203"))
        tabs.push("heroesTab");
    if (AreaManager.idToArea("A01").unlocked())
        tabs.push("dungeonsTab");
    if (QuestManager.unlocked())
        tabs.push("questsTab");
...
function tabHide() {
    if (recipeList.idToItem("R13001").craftCount > 0)
        $inventoryTabLink.show();
    else
        $inventoryTabLink.hide();
    if (Shop.alreadyPurchased("AL1000"))
        $guildTabLink.show();
    else
        $guildTabLink.hide();
    if (HeroManager.heroOwned("H203"))
        ...
        $heroesTabLink.hide();
    if (AreaManager.idToArea("A01").unlocked())
        $dungeonsTabLink.show();
    else
        $dungeonsTabLink.hide();
    if (QuestManager.unlocked())
        $questsTabLink.show();
    else
        $questsTabLink.hide();
...
var Item = function() {
    ...
    _createClass(Item, [{
        ...
        value: function loadSave(save) {
            ...
            if (save.mastered !== undefined)
                this.mastered = save.mastered;
            if (save.museum !== undefined)
                this.museum = save.museum
        }
    }, {
        key: "itemDescription",
        ...
        value: function recipeListStats() {
            ...
            if (pow > 0)
                $("<div/>").addClass("recipeStatListPow tooltip").attr("data-tooltip", "pow").html("".concat(miscIcons.pow, '<span class="statValue">').concat(pow, "</span>")).appendTo(d);
            if (hp > 0)
                $("<div/>").addClass("recipeStatListHP tooltip").attr("data-tooltip", "hp").html("".concat(miscIcons.hp, '<span class="statValue">').concat(hp, "</span>")).appendTo(d);
            return d
        }
    }, {
        key: "count",
        ...
        value: function isMastered() {
            if (this.recipeType === "building" || this.recipeType === "Trinket")
                return false;
        ...
        value: function autoSellToggle() {
            if (this.autoSell === "None")
                this.autoSell = "Common";
            else if (this.autoSell === "Common")
                this.autoSell = "Good";
            else if (this.autoSell === "Good")
                this.autoSell = "Great";
            else if (this.autoSell === "Great")
                this.autoSell = "Epic";
            else
                this.autoSell = "None";
...
var recipeList = {
    ...
    filteredRecipeList: function filteredRecipeList() {
        ...
        if (this.recipeFilterType === "default")
            return this.recipes.filter(function(r) {
                return r.owned && r.name.toLowerCase().includes(cleanString)
            });
        if (this.masteryFilter === MasteryFilter.BOTH && this.recipeFilterScope === "level")
            return this.recipes.filter(function(r) {
                return r.owned && r.recipeType === "normal" && r.lvl === _this2.recipeFilterType
            });
        if (this.masteryFilter === MasteryFilter.UNMASTERED && this.recipeFilterScope === "level")
            return this.recipes.filter(function(r) {
                return r.owned && !r.mastered && r.recipeType === "normal" && r.lvl === _this2.recipeFilterType
            });
        if (this.masteryFilter === MasteryFilter.MASTERED && this.recipeFilterScope === "level")
            return this.recipes.filter(function(r) {
                return r.owned && r.mastered && r.recipeType === "normal" && r.lvl === _this2.recipeFilterType
            });
        if (this.masteryFilter === MasteryFilter.BOTH && this.recipeFilterScope === "itemType")
            return this.recipes.filter(function(r) {
                return r.owned && r.type === _this2.recipeFilterType
            });
        if (this.masteryFilter === MasteryFilter.UNMASTERED && this.recipeFilterScope === "itemType")
            return this.recipes.filter(function(r) {
                return r.owned && !r.mastered && r.type === _this2.recipeFilterType
            });
        if (this.masteryFilter === MasteryFilter.MASTERED && this.recipeFilterScope === "itemType")
            return this.recipes.filter(function(r) {
                return r.owned && r.mastered && r.type === _this2.recipeFilterType
            })
    },
    ...
    if (recipeList.recipeFilterScope === "itemType") {
        ItemType.forEach(function(itemtype) {
            if (recipeList.ownAtLeastOne(itemtype)) {
                ...
                if (itemtype === "Knives")
                    recipeSelect.addClass("selectedRecipeFilter");
...
function recipeCardFront(recipe) {
    ...
    if (recipe.recipeType !== "normal")
        recipeStats.append(timeStat);
    else
        recipeStats.append(gearStat, goldStat, timeStat);
    ...
    if (recipe.isMastered())
        td6a.addClass("isMastered").html('<i class="fas fa-star-christmas"></i> '.concat(displayText("recipes_card_mastery_recipe_mastered")));
...
function refreshCraftedCount(recipe) {
    if (!recipe.rbd)
        recipe.rbd = $("#rbd" + recipe.id);
    if (!recipe.rms)
        recipe.rms = $("#rms" + recipe.id);
    if (!recipe.rcd)
        recipe.rcd = $("#rcd" + recipe.id);
...
function recipeCanCraft() {
    recipeList.recipes.forEach(function(recipe) {
        if (recipe.recipeDiv === undefined)
            return;
        if (recipe.canProduce && actionSlotManager.slots.length < actionSlotManager.maxSlots)
            recipe.recipeDiv.removeClass("recipeCraftDisable");
        else
            recipe.recipeDiv.addClass("recipeCraftDisable")
    })
}
...
$(document).on("click", ".recipeSortButton", function(e) {
    ...
    if (searchString.length < 2)
        return Notifications.popToast("search_length_invalid");
...
$(document).on("click", ".recipeSelect", function(e) {
    e.preventDefault();
    if (recipeList.recipeFilterType === $(e.currentTarget).data("itemType") || recipeList.recipeFilterType === $(e.currentTarget).data("level"))
        return;
...
$(document).on("click", ".recipeScopeName", function(e) {
    e.preventDefault();
    if (recipeList.recipeFilterScope === $(e.currentTarget).data("recipeScope"))
        return;
    recipeList.recipeFilterScope = $(e.currentTarget).data("recipeScope");
    if (recipeList.recipeFilterScope === "level")
        recipeList.recipeFilterType = 1;
    else if (recipeList.recipeFilterScope === "itemType")
...
var ResourceManager = {
    ...
    canAffordMaterial: function canAffordMaterial(item) {
        ...
        for (var _i = 0, _Object$entries = Object.entries(item.mcost); _i < _Object$entries.length; _i++) {
            var _Object$entries$_i = _slicedToArray(_Object$entries[_i], 2)
              , material = _Object$entries$_i[0]
              , amt = _Object$entries$_i[1];
            if (amt > this.materialAvailable(material))
                return false
        }
        return true
    },
    ...
    materialIcon: function materialIcon(type) {
        if (type[0] === "R")
            return recipeList.idToItem(type).itemPic();
        if (type[0] === "G")
            return GuildManager.idToGuild(type).icon;
    ...
    nameForWorkerSac: function nameForWorkerSac(mat) {
        ...
        if (item === undefined)
            return this.idToMaterial(mat).name;
        return item.name
    },
...
function hardMatRefresh() {
    if (ResourceManager.materialsEmpty())
        $noMaterialDiv.show();
    else
        $noMaterialDiv.hide();
    ResourceManager.materials.forEach(function(mat) {
...
var CombatManager = {
    ...
    execute: function execute(combatRound) {
        ...
        if (combatRound.attack.id !== "S0000")
            combatRound.attacker.buffTick("onSpecial")
    }
};
    ...
    _createClass(combatRoundParams, [{
        ...
        value: function getTarget(target, side) {
            ...
            if (target === TargetType.MISSINGHP)
                return [living.reduce(function(a, b) {
            ...
            if (target === TargetType.ADJACENT) {
                ...
                var targets = [];
                if (_indx4 !== living.length - 1)
                    targets.push(living[_indx4 + 1]);
                if (_indx4 !== 0)
                    targets.push(living[_indx4 - 1]);
                return targets
            }
            if (target === TargetType.MIRROR) {
                ...
                if (this.attacker.confusion())
                    return [this.attacker];
                if (enemies.length <= _indx5)
                    _indx5 = enemies.length - 1;
                if (this.enemies[_indx5].alive())
                    return [this.enemies[_indx5]];
                if (_indx5 > 0 && this.enemies[_indx5 - 1].alive())
                    return [this.enemies[_indx5 - 1]];
                if (_indx5 < this.enemies.length - 1 && this.enemies[_indx5 + 1].alive())
                    return [this.enemies[_indx5 + 1]];
                if (_indx5 - 1 > 0 && this.enemies[_indx5 - 2].alive())
                    return [this.enemies[_indx5 - 2]];
                if (_indx5 < this.enemies.length - 2 && this.enemies[_indx5 + 2].alive())
                    return [this.enemies[_indx5 + 2]];
                if (_indx5 - 2 > 0 && this.enemies[_indx5 - 3].alive())
                    return [this.enemies[_indx5 - 3]];
                if (_indx5 < this.enemies.length - 3 && this.enemies[_indx5 + 3].alive())
                    return [this.enemies[_indx5 + 3]]
            }
...
var Combatant = function() {
    ...
    _createClass(Combatant, [{
        ...
        value: function takeAttack(attack) {
            ...
            refreshHPBar(this);
            if (this.thorns() > 0)
                attack.attacker.takeDamage(this.thorns());
            if (this.parry() > 0)
                attack.attacker.takeDamage(this.parry());
            if (this.beornTank() > 0) {
...
var Buff = function() {
    ...
    _createClass(Buff, [{
        ...
        value: function buffTick(type, attack, refreshLater) {
            ...
            } else if (!refreshLater)
                BuffRefreshManager.updateBuffCount(this, this.target)
        }
    }, {
        key: "expired",
    ...
    hardRefreshBuff: function hardRefreshBuff() {
        ...
        dungeon.party.heroes.forEach(function(ally) {
            ...
            ally.buffs.forEach(function(buff) {
                _this2.makeBuffContainer(buff, ally.uniqueid).appendTo($heroDiv)
            })
        });
        dungeon.mobs.forEach(function(enemy) {
            ...
            enemy.buffs.forEach(function(buff) {
                _this2.makeBuffContainer(buff, enemy.uniqueid).appendTo($enemyDiv)
            })
        })
    },
...
var B0022 = function(_Buff6) {
    ...
    function B0022(buffTemplate, target, power, power2) {
        ...
        _this3 = _possibleConstructorReturn(this, _getPrototypeOf(B0022).call(this, buffTemplate, target, power));
        _this3.power2 = power2;
        return _this3
    }
    _createClass(B0022, [{
        key: "maxHPPercent",
...
var Hero = function(_Combatant) {
    ...
    _createClass(Hero, [{
        ...
        value: function getPow(noBuff) {
            if (noBuff)
                return this.initialPow + this.gearSlots.map(function(g) {
                    return g.pow()
                }).reduce(function(a, b) {
                    return a + b
                });
        ...
        value: function maxHP(noBuff) {
            if (noBuff)
                return this.initialHP + this.gearSlots.map(function(g) {
                    return g.hp()
                }).reduce(function(a, b) {
                    return a + b
                });
        ...
        value: function getEquipSlots(nonblank) {
            if (nonblank)
                return this.gearSlots.map(function(g) {
                    return g.gear
                }).filter(function(s) {
                    return s !== null
                });
...
var gearSlot = function() {
    ...
    _createClass(gearSlot, [{
        ...
        value: function createSave() {
            var save = {};
            if (this.gear !== null)
                save.gear = this.gear.createSave();
            else
                save.gear = null;
            save.lvl = this.lvl;
            return save
...
var HeroManager = {
    ...
    upgradeSlot: function upgradeSlot(heroID, gearType) {
        if (this.totalUpgrades() >= DungeonManager.availableUpgrades())
            return Notifications.popToast("cant_afford_slot_upgrade");
        var hero = this.idToHero(heroID);
...
function refreshHeroOverview() {
    ...
    if (DungeonManager.bossRefightUnlocked())
        $heroTabUpgrade.removeClass("heroTabLocked").html(displayText("hero_tab_upgrade"));
    else
        $heroTabUpgrade.addClass("heroTabLocked").html("".concat(miscIcons.locked, "&nbsp;").concat(displayText("universal_locked")));
    if (TinkerManager.unlocked())
        $heroTabTrinket.removeClass("heroTabLocked").html(displayText("hero_tab_trinket"));
    else
        $heroTabTrinket.addClass("heroTabLocked").html("".concat(miscIcons.locked, "&nbsp;").concat(displayText("universal_locked")))
}
...
function cyclePreviewHeroPrevious() {
    ...
    if (heroIndex > 0)
        ...
        newHeroIndex = heroesList.length - 1;
...
function cyclePreviewHeroNext() {
    ...
    var newHeroIndex = 0;
    if (heroIndex < heroesList.length - 1)
        newHeroIndex = heroIndex + 1;
    HeroManager.heroView = heroesList[newHeroIndex].id
}
...
function generateHeroPlaybooks(hero) {
    ...
    hero.playbooks.forEach(function(playbookID) {
        ...
        if (hero.playbook.id === playbookID)
            d.addClass("playbookSelected");
...
function showHeroGear() {
    ...
    hero.gearSlots.forEach(function(slot) {
        if (slot.type !== "Trinkets")
            heroCurrentGearEquip(hero, slot).appendTo($heroGearSlotList)
    });
...
function showHeroUpgrades() {
    ...
    hero.gearSlots.forEach(function(slot) {
        if (slot.type !== "Trinkets")
            heroUpgradeSlot(HeroManager.heroView, slot).appendTo($heroUpgradesGear)
    })
}
...
function refreshTrinketInventory() {
    ...
    if (Inventory.listbyType("Trinkets").length === 0)
        $("<div/>").addClass("heroUpgradeTrinketsText").html(displayText("no_trinkets_desc")).appendTo($heroEquipTrinketAll);
...
function heroCurrentGearEquip(hero, gearSlot) {
    ...
    if (hero.equipUpgradeAvailable(type))
        d.addClass("equipUpgradeAvailable");
...
function heroUpgradeSlot(heroid, gearSlot) {
    ...
    if (gearSlot.lvl == 2)
        d1.html("Max Level").addClass("heroGearUpgradeSlotButtonMax");
    else
        d1.html("Upgrade").addClass("heroGearUpgradeSlotButton");
    return d
}
...
function examineHeroPossibleEquip(heroID, gearType, skipAnimation) {
    ...
    Inventory.listbyType(gearType).forEach(function(itemContainer) {
        if (currentTypes.includes(itemContainer.uniqueID()))
            return;
...
$(document).on("click", ".heroTab", function(e) {
    e.preventDefault();
    if ($(e.currentTarget).hasClass("heroTabLocked"))
        return;
...
function startPartyCreation(partyStarted) {
    ...
    if (PartyCreator.dungeonSelect === null) {
        if (area.lastVisitedDungeon === null)
            PartyCreator.setDungeon(area.lastOpen().id);
        else
            PartyCreator.setDungeon(area.lastVisitedDungeon)
    }
    ...
    if (!partyStarted) {
        ...
        if (dungeon.type === "boss" && !dungeon.beaten())
            $("<div/>").attr("id", "dungeonTeamButtonBoss").addClass("dungeonTeamButton actionButton").html(displayText("adventure_launch_floor_boss")).appendTo(partyLaunch);
        else if (dungeon.type === "boss" && !DungeonManager.bossRefightUnlocked())
            $("<div/>").addClass("dungeonTeamButtonLocked").html("".concat(miscIcons.locked, " ").concat(displayText("universal_locked"))).appendTo(partyLaunch);
        else if (dungeon.type === "boss" && DungeonManager.bossRefightUnlocked()) {
    ...
    area.dungeons.forEach(function(dungeon) {
        if (!dungeon.unlocked())
            return;
        var d = $("<div/>").addClass("dtsDungeon").data("dungeonID", dungeon.id).prependTo(dtsDungeons);
        if (PartyCreator.dungeonSelect === dungeon.id)
            d.addClass("dtsHighlight");
        if (dungeon.mat !== null) {
    ...
    HeroManager.ownedHeroes().forEach(function(hero) {
        if (hero.state === HeroState.inDungeon)
            characterCard("dungeonNotAvailable", hero.uniqueid, hero.id, "in_dungeon").appendTo(d2);
        else if (hero.state === HeroState.inQuest)
            characterCard("dungeonNotAvailable", hero.uniqueid, hero.id, "in_quest").appendTo(d2);
        else if (PartyCreator.heroes.includes(hero.id))
            characterCard("partyHero dungeonNotAvailable", hero.uniqueid, hero.id, "in_party").appendTo(d2);
        else
            characterCard("dungeonAvailable", hero.uniqueid, hero.id, null).appendTo(d2)
    });
    $dtsBottom.append(d2)
}
...
$(document).on("click", ".dtsDungeon", function(e) {
    ...
    if (PartyCreator.dungeonSelect === dungeonid)
        return;
...
function setHeroDialogOpen(heroID) {
    ...
    if (hero.description)
        $("<div/>").addClass("dialogDescription").html(hero.description).appendTo(dialogContentContainer);
...
$(document).on("click", ".heroPlaybook", function(e) {
    ...
    var heroID = $(e.currentTarget).data("heroID");
    if (settings.dialogStatus === 0 && id)
        setHeroDialogOpen(heroID)
});
...
var TurnOrder = function() {
    ...
    _createClass(TurnOrder, [{
        ...
        value: function positionInParty() {
            ...
            if (huid.includes(uniqueid))
                return huid.findIndex(function(h) {
                    return h === uniqueid
                });
...
var Area = function() {
    ...
    _createClass(Area, [{
        ...
        key: "loadSave",
        value: function loadSave(save) {
            if (save.lastVisitedDungeon !== undefined)
                this.lastVisitedDungeon = save.lastVisitedDungeon;
            return
        }
    }, {
        key: "unlocked",
...
var Dungeon = function() {
    ...
    _createClass(Dungeon, [{
        ...
        value: function createSave() {
            ...
            if (this.order !== null)
                save.order = this.order.createSave();
            else
                save.order = null;
        ...
        value: function loadSave(save) {
            var _this2 = this;
            if (save.party)
                this.party = new Party(save.party.heroID);
            if (save.mobs)
            ...
            if (save.maxFloor !== undefined)
                this.maxFloor = save.maxFloor;
            if (save.floor !== undefined)
                this.floor = save.floor;
            if (save.floorClear !== undefined)
                this.floorClear = save.floorClear;
            if (save.order) {
        ...
        value: function addTime(t, uncapMats) {
            ...
            while (this.dungeonTime >= dungeonWaitTime) {
                ...
                if (!refreshLater && DungeonManager.dungeonView === this.id)
                    $("#beatbarFill".concat(this.order.getCurrentID())).css("width", "0%");
            ...
            if (DungeonManager.dungeonView === this.id)
                refreshBeatBar(this.order.getCurrentID(), this.dungeonTime)
        }
    }, {
        key: "setMobIDs",
        value: function setMobIDs() {
            ...
            if (this.mob2 !== null)
                this.mobIDs.push(this.mob2);
            if (this.mob3 !== null)
                this.mobIDs.push(this.mob3);
            if (this.mob4 !== null)
                this.mobIDs.push(this.mob4)
        }
    }, {
        key: "addDungeonReward",
        value: function addDungeonReward(time, skipAnimation, uncapMats) {
            if (this.type === "boss" || this.floorClear === 0)
                return;
        ...
        value: function resetDungeon() {
            if (this.status !== DungeonStatus.ADVENTURING && this.status !== DungeonStatus.SUCCESS && this.status !== DungeonStatus.FAILURE)
                return;
        ...
        value: function previousFloor(refreshLater) {
            if (this.type === "boss")
                return this.status = DungeonStatus.FAILURE;
        ...
        value: function nextFloor(refreshLater) {
            if (this.type === "boss") {
                ...
                if (DungeonManager.dungeonView === this.id)
                    initiateDungeonFloor(this.id, refreshLater);
        ...
        value: function unlocked() {
            if (this.unlockedBy === null)
                return true;
            if (this.unlockedBy.charAt(0) === "A")
                return Shop.alreadyPurchased(this.unlockedBy);
            if (!Shop.alreadyPurchased("AL20081"))
                return false;
        ...
        key: "addMob",
        value: function addMob(mobID) {
            var position = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 999;
...
function refreshDungeonDead(uniqueid) {
    if (!DungeonManager.dungeonView)
        return;
    var dungeon = DungeonManager.dungeonByID(DungeonManager.dungeonView);
    if (dungeon.mobs.map(function(m) {
        return m.uniqueid
    }).includes(uniqueid))
        $("#dfm" + uniqueid).addClass("mobDead");
    if (dungeon.party.heroes.map(function(h) {
        return h.uniqueid
    }).includes(uniqueid))
        $("#dfc" + uniqueid).addClass("heroDead")
}
"use strict";
...
function createAreaBlock(area) {
    ...
    if (area.status() === DungeonStatus.SUCCESS)
        d1.addClass("statusSuccess");
    if (area.status() === DungeonStatus.FAILURE)
        d1.addClass("statusFailure");
...
function initiateDungeonFloor(dungeonID) {
    if (DungeonManager.dungeonView !== dungeonID)
        return;
    var dungeon = DungeonManager.dungeonByID(DungeonManager.dungeonView);
...
function generatePassiveSkill(unit) {
    ...
    if (passiveID)
        $("<div/>").addClass("orderUnitPassive tooltip").attr({
            id: "oUP" + unit.id,
            "data-tooltip": "skill_desc",
...
function initializeSideBarDungeon() {
    ...
    AreaManager.areas.forEach(function(area) {
        if (area.type === "boss" || !area.unlocked())
            return;
...
var WorkerManager = {
    ...
    canCurrentlyCraft: function canCurrentlyCraft(item) {
        ...
        for (var _i2 = 0, _Object$entries2 = Object.entries(needBucket); _i2 < _Object$entries2.length; _i2++) {
            var _Object$entries2$_i = _slicedToArray(_Object$entries2[_i2], 2)
              , res = _Object$entries2$_i[0]
              , amt = _Object$entries2$_i[1];
            if (this.canProduceBucket[res] === undefined || this.canProduceBucket[res] < amt)
                return false
        }
        return true
    },
    ...
    freeByGuild: function freeByGuild(gid) {
        ...
        if (usage[gid] === undefined)
            return this.ownedByGuild(gid);
...
function refreshSideWorkers() {
    ...
    if (g1free > 0)
        $G001WorkersSide.removeClass("noWorkersAvailable");
    else
        $G001WorkersSide.addClass("noWorkersAvailable");
    if (g2free > 0)
        $G002WorkersSide.removeClass("noWorkersAvailable");
    else
        $G002WorkersSide.addClass("noWorkersAvailable");
    if (g3free > 0)
        $G003WorkersSide.removeClass("noWorkersAvailable");
    else
        $G003WorkersSide.addClass("noWorkersAvailable");
    if (g4free > 0)
        $G004WorkersSide.removeClass("noWorkersAvailable");
    else
        $G004WorkersSide.addClass("noWorkersAvailable");
    if (WorkerManager.ownedByGuild("G001") > 0)
        $G001WorkersSide.show();
    else
        $G001WorkersSide.hide();
    if (WorkerManager.ownedByGuild("G002") > 0)
        $G002WorkersSide.show();
    else
        $G002WorkersSide.hide();
    if (WorkerManager.ownedByGuild("G003") > 0)
        $G003WorkersSide.show();
    else
        $G003WorkersSide.hide();
    if (WorkerManager.ownedByGuild("G004") > 0)
        $G004WorkersSide.show();
    else
        $G004WorkersSide.hide()
}
"use strict";
function _slicedToArray(arr, i) {
    return _arrayWithHoles(arr) || _iterableToArrayLimit(arr, i) || _nonIterableRest()
...
var itemContainer = function() {
    ...
    _createClass(itemContainer, [{
        ...
        value: function loadSave(save) {
            ...
            if (save.scale !== undefined)
                this.scale = save.scale;
            if (save.powRatio !== undefined)
                this.powRatio = save.powRatio;
            if (save.hpRatio !== undefined)
                this.hpRatio = save.hpRatio
        }
    }, {
        key: "picName",
        value: function picName() {
            ...
            if (!sharp)
                return "".concat(this.item.itemPic(), '<div class="item-prefix-name">').concat(this.prefix()).concat(this.item.name, "</div>");
        ...
        value: function itemLevel() {
            if (this.scale > 0)
                return '<div class="level_text">'.concat(miscIcons.star, '</div><div class="level_integer">').concat(this.scale, "</div>");
            return '<div class="level_text">LVL</div><div class="level_integer">'.concat(this.lvl, "</div>")
        }
    }, {
        key: "pow",
        ...
        value: function prefix() {
            if (this.powRatio === this.item.pow && this.hpRatio === this.item.hp)
                return "";
...
var Inventory = {
    ...
    addToInventory: function addToInventory(container, skipAnimation) {
        if (this.full())
            this.sellContainer(container, skipAnimation);
        else {
            this.findempty(container, skipAnimation);
    ...
    craftToInventory: function craftToInventory(id, skipAnimation) {
        if (TownManager.buildingRecipes().includes(id))
            return TownManager.unlockBldg(id);
        ...
        if (roll < procRate.epic) {
            ...
            if (sellToggle < 3) {
                ...
                this.sellContainer(greatItem, skipAnimation);
            ...
            if (sellToggle < 2) {
                ...
                this.sellContainer(goodItem, skipAnimation);
            achievementStats.craftedItem("Good")
        } else {
            var commonItem = new itemContainer(id,0);
            if (sellToggle < 1)
                this.addToInventory(commonItem, skipAnimation);
            else
                this.sellContainer(commonItem, skipAnimation);
    ...
    sellCommons: function sellCommons() {
        ...
        this.inv.forEach(function(ic, indx) {
            if (ic !== null && ic.rarity === 0 && ic.item.recipeType === "normal")
                _this2.sellInventoryIndex(indx, true)
        });
        refreshInventoryPlaces()
    },
...
function gearEquipFromInventory(invID) {
    ...
    heroes.forEach(function(hero) {
        ...
        for (var _i5 = 0, _Object$entries5 = Object.entries(newStats); _i5 < _Object$entries5.length; _i5++) {
            ...
            if (deltaStat > 0)
                d4a.addClass("hebPositive").html("".concat(miscIcons[stat], ' <span class="statValue">').concat(val, " (+").concat(deltaStat, ")</span>"));
            else if (deltaStat < 0)
                d4a.addClass("hebNegative").html("".concat(miscIcons[stat], ' <span class="statValue">').concat(val, " (").concat(deltaStat, ")</span>"));
            else
                d4a.html("".concat(miscIcons[stat], ' <span class="statValue">').concat(val))
        }
        if (same)
            $("<div/>").addClass("heroEquipBlockEquipStat").html("No Change").appendTo(d4);
...
function refreshInventoryPlaces() {
    if (lastTab === "inventoryTab")
        refreshInventory();
    if (lastTab === "guildTab")
        refreshOrderInvCount();
    if (lastTab === "townsTab" && TownManager.lastBldg === "fusion")
        refreshPossibleFuse();
    if (lastTab === "townsTab" && TownManager.lastBldg === "bank")
        refreshBankPage();
    if (lastTab === "townsTab" && TownManager.lastBldg === "smith") {
        refreshSmithInventory();
        refreshSmithStage()
    }
    if (lastTab === "townsTab" && TownManager.lastBldg === "synth")
        refreshSynthInventory();
    if (lastTab === "townsTab" && TownManager.lastBldg === "fortune")
        refreshFortuneGear();
    if (lastTab === "townsTab" && TownManager.lastBldg === "museum")
        refreshMuseumInv();
    if (lastTab === "townsTab" && TownManager.lastBldg === "forge")
        refreshSmithInventory();
    $sideInventoryAmt.html("".concat(Inventory.inventoryCount(), "/20"));
    if (Inventory.inventoryCount() === 20)
        $sideInventory.addClass("inventoryFullSide");
    else
        $sideInventory.removeClass("inventoryFullSide")
}
"use strict";
function _toConsumableArray(arr) {
    return _arrayWithoutHoles(arr) || _iterableToArray(arr) || _nonIterableSpread()
...
var actionSlot = function() {
    ...
    _createClass(actionSlot, [{
        ...
        value: function addTime(t, skipAnimation) {
            if (this.status === slotState.NEEDMATERIAL)
        ...
        value: function attemptStart(skipAnimation) {
            ...
            if (!ResourceManager.canAffordMaterial(this.item))
                return;
        ...
        value: function refundMaterial() {
            if (this.status !== slotState.CRAFTING || this.item.isMastered())
                return;
...
var actionSlotManager = {
    ...
    addSlot: function addSlot(itemid) {
        ...
        if (item.recipeType !== "normal" && this.isAlreadySlotted(itemid))
            return;
        if (!item.owned)
            return Notifications.popToast("recipe_not_owned");
        if (!item.canProduce) {
    ...
    showAutoSell: function showAutoSell() {
        this.slots.forEach(function(slot) {
            if (slot.item.recipeType === "normal")
                $("#asAuto".concat(slot.slotNum)).show()
        })
    }
};
...
function newActionSlot(slot) {
    ...
    if (slot.status === slotState.NEEDMATERIAL)
        d3a.addClass("matsNeeded").attr({
    ...
    if (slot.isMastered())
        s3.addClass("ProgressBarFillMaster");
    ...
    if (slot.autoSell() !== "None")
        d4.addClass("ASautoEnabled" + slot.autoSell());
    if (slot.isBuildingMaterial() || !Shop.alreadyPurchased("AL3000"))
        d4.hide();
    if (!slot.resList)
        return d;
...
var fuse = function() {
    ...
    _createClass(fuse, [{
        ...
        key: "loadSave",
        value: function loadSave(save) {
            if (save.fuseTime !== undefined)
                this.fuseTime = save.fuseTime;
            if (save.started !== undefined)
...
var FusionManager = {
    ...
    loadSave: function loadSave(save) {
        ...
        if (save.lvl !== undefined)
            this.lvl = save.lvl
    },
    addFuse: function addFuse(uniqueid) {
        if (!Inventory.hasThree(uniqueid))
            return;
        if (this.slots.length === this.maxSlots()) {
    ...
    collectFuse: function collectFuse(fuseID) {
        ...
        if (slot === undefined || !slot.fuseComplete())
            return;
        if (Inventory.full()) {
            Notifications.popToast("fuse_inv_full");
            return
...
function refreshFuseBars() {
    FusionManager.slots.forEach(function(fuse) {
        if (fuse.fbv === undefined)
            fuse.fbv = $("#fuseBarDiv" + fuse.fuseID);
        if (fuse.fsc === undefined)
            fuse.fsc = $("#fuseSlotCollect" + fuse.fuseID);
        if (fuse.fb === undefined)
            fuse.fb = $("#fuseBar" + fuse.fuseID);
        if (fuse.ff === undefined)
            fuse.ff = $("#fuseFill" + fuse.fuseID);
...
function refreshFuseHaveEvent() {
    ...
    if ($fusionBldg === undefined)
        $fusionBldg = $("#fusionBldg");
    if (completed) {
...
function refreshBankInventory() {
    ...
    if (Inventory.nonblank().length === 0)
        $("<div/>").addClass("emptyContentMessage").html(displayText("bank_inventory_empty")).appendTo($bankInvSlots);
...
function refreshBankBank() {
    ...
    if (BankManager.slots.length === 0)
        $("<div/>").addClass("emptyContentMessage").html(displayText("bank_storage_empty")).appendTo($bankBankSlots);
...
function itemCard(item, inBank) {
    ...
    if (item.item.recipeType === "building")
        itemLevel.hide();
    ...
    if (inBank)
        locationButton.addClass("actionButtonCard bankTake").html(displayText("bank_remove_item_button"));
    else
        locationButton.addClass("actionButtonCard bankStow").html(displayText("bank_stow_item_button"));
...
var bloopSmith = {
    ...
    addSmith: function addSmith(containerID, location) {
        ...
        if (item.sharp >= this.maxSharp())
            return Notifications.popToast("cant_smith_max");
...
function itemStageCardSmith(slot, upgrade) {
    ...
    var itemName = $("<div/>").addClass("itemName");
    if (upgrade)
        itemName.html(slot.picNamePlus());
    else
        itemName.html(slot.picName());
...
var fortuneSlot = function() {
    function fortuneSlot(container) {
        ...
        if (container.rarity === 0)
            this.amt = 20;
        else if (container.rarity === 1)
            this.amt = 50;
        else if (container.rarity === 2)
            this.amt = 100;
        fortuneSlotid += 1
    }
...
function refreshFortuneSlots() {
    ...
    FortuneManager.slots.forEach(function(slot) {
        if (slot.locked())
            ...
            $fortuneStage.append(createFortuneCard(slot))
    });
...
var SynthManager = {
    ...
    stageButton: function stageButton(id) {
        if (this.slot !== null && this.setting === synthToggle.DESYNTH)
            this.desynth();
        if (this.slot !== null && this.setting === synthToggle.RESYNTH)
            this.resynth(id)
    },
    ...
    desynthRewards: function desynthRewards(increase) {
        ...
        if (this.slot === null)
            return null;
        var reward = {};
        if (this.slot.rarity + mod === 1)
            reward.id = "M700";
        if (this.slot.rarity + mod === 2)
            reward.id = "M701";
        if (this.slot.rarity + mod === 3)
            reward.id = "M702";
        reward.amt = this.amt();
        return reward
    },
...
function refreshSynthInventory() {
    ...
    if (Inventory.higherRarity().length === 0)
        $("<div/>").addClass("emptyContentMessage").html(displayText("synthesizer_inventory_empty")).appendTo($synthListContainer);
    Inventory.higherRarity().forEach(function(container) {
        if (container.item.type === "Trinkets" || container === SynthManager.slot)
            return;
...
function refreshSynthButtons() {
    if (SynthManager.canSynth())
        $("#synthPowerResynthesis").removeClass("synthPowerSettingLocked").html(displayText("synthesizer_synth_setting"))
}
...
function createSynthStageCard(container) {
    ...
    if (SynthManager.setting === synthToggle.DESYNTH)
        synthButton.addClass("actionButtonCard").html(displayText("synthesizer_desynth_assign_button"));
...
var tinkerCommand = function() {
    ...
    _createClass(tinkerCommand, [{
        ...
        value: function attemptStart() {
            if (this.paidGold)
                return true;
            if (!ResourceManager.available("M001", this.paidGoldAmt()))
                return false;
        ...
        value: function completeResearch() {
            if (!ResourceManager.available("M001", this.completeCost()))
                return Notifications.popToast("tinker_research");
...
function refreshSideTown() {
    ...
    TownManager.buildings.forEach(function(building) {
        if (building.getStatus() >= 0) {
            ...
            if (TownManager.lastBldg === building.shorthand)
                d.addClass("selected");
            if (building.getStatus() === BuildingState.unseen)
                d.addClass("hasEvent");
...
function showBldg(type) {
    ...
    if (building.getStatus() === BuildingState.built)
        db.html("<img src='assets/images/townImages/".concat(building.shorthand, "Building/").concat(building.shorthand, "_building.png'>"));
...
$(document).on("click", ".buildingName", function(e) {
    ...
    if (TownManager.lastBldg === type)
        return;
    TownManager.lastBldg = type;
    var building = TownManager.typeToBuilding(type);
    if (building.getStatus() === BuildingState.unseen)
        building.setStatus(BuildingState.seen);
    $(".buildingName").removeClass("selected");
    if (!TownManager.unseenLeft())
        $("#townTab").removeClass("hasEvent");
    $(e.currentTarget).addClass("selected");
...
var Guild = function() {
    ...
    _createClass(Guild, [{
        ...
        value: function loadSave(save) {
            ...
            if (save.unmastered !== undefined)
                this.unmastered = save.unmastered
        }
    }, {
        key: "addRep",
        value: function addRep(rep) {
            if (this.maxLvlReached())
                return;
            this.rep += rep;
        ...
        value: function generateNewOrder(orderNum) {
            ...
            if (orderNum === 1) {
                ...
                if (possibleGuildItems.length > 1)
                    possibleGuildItems = possibleGuildItems.filter(function(r) {
            ...
            if (possibleItems.length > 1)
                possibleItems = possibleItems.filter(function(r) {
                    return r.id !== previous
                });
            var chosenItem = possibleItems[Math.floor(GuildSeedManager.fauxRand(this.id) * possibleItems.length)];
            if (orderNum === 2)
                this.order2 = new guildOrderItem(this.id,chosenItem.id,this.lvl);
            if (orderNum === 3)
                this.order3 = new guildOrderItem(this.id,chosenItem.id,this.lvl)
        }
    }, {
        key: "submitItem",
        value: function submitItem(slot) {
            ...
            if (submitContainer.complete())
                this.generateNewOrder(slot, submitContainer.id);
            refreshAllOrders()
        }
    }, {
        key: "goldValue",
...
var guildOrderItem = function() {
    ...
    _createClass(guildOrderItem, [{
        ...
        value: function generateRarity(lvl) {
            ...
            if (epicChance > rarityRoll)
                return 3;
            if (greatChance > rarityRoll)
                return 2;
            if (goodChance > rarityRoll)
                return 1;
            return 0
        }
    }, {
        ...
        value: function generateSharp(lvl) {
            ...
            if (sharpChance < Math.floor(GuildSeedManager.fauxRand(this.gid) * 100))
                return 0;
        ...
        value: function generateName() {
            if (this.sharp > 0)
                return '<span><span class="item-prefix tooltip" data-tooltip="forge_level">+'.concat(this.sharp, "</span>").concat(this.item.name, "</span>");
...
function initializeGuilds() {
    ...
    GuildManager.guilds.forEach(function(g) {
        ...
        if (GuildManager.lastClicked === g.id)
...
function checkCraftableStatus() {
    ...
    recipeList.recipes.forEach(function(recipe) {
        if (!recipe.canProduce || !recipe.owned || actionSlotManager.slots.length >= actionSlotManager.maxSlots)
            $("#" + recipe.id + ".orderCraft").addClass("recipeCraftDisable")
    })
}
...
function generateProgressBar(options) {
    ...
    if (tooltip)
        progressBarContainer.addClass("tooltip").attr({
            "data-tooltip": tooltip
        });
    var progressBarText = $("<div/>").addClass("progressBarText");
    if (text)
        progressBarText.html(text).appendTo(progressBarContainer);
    if (textID)
        progressBarText.attr({
            id: textID
        });
    var progressBarContent = $("<div/>").addClass("progressBarContent");
    if (icon)
        $("<div/>").addClass("progressBarIcon").html(icon).appendTo(progressBarContent);
    if (icon && text)
        progressBarText.addClass("containsIcon");
...
function createOrderCard(item, id, index) {
    ...
    if (item.item.mcost) {
        Object.keys(item.item.mcost).forEach(function(mat, i) {
            ...
            if (item.item.isMastered())
                matBox.addClass("isMastered")
        })
    }
    ...
    if (Inventory.itemCountSpecific(item.uniqueID()) > 0)
        invCount.addClass("canContribute");
...
function refreshOrderInvCount() {
    $(".orderInv").each(function() {
        ...
        if (invCount > 0)
            $this.addClass("canContribute").html('<i class="fas fa-cube"></i> '.concat(invCount));
        else
...
var Perk = function() {
    ...
    _createClass(Perk, [{
        ...
        value: function purchase() {
            this.purchased = true;
            if (this.type === "hero")
                HeroManager.gainHero(this.subtype);
            if (this.type === "worker") {
                refreshSideWorkers();
                refreshRecipeFilters();
                recipeList.canCraft();
            ...
            if (this.type === "autosell")
                actionSlotManager.showAutoSell();
            if (this.type === "craft")
                actionSlotManager.upgradeSlot();
            if (this.type === "synth" && this.subtype === "open")
                TownManager.buildingPerk("synth");
            if (this.type === "bank" && this.subtype === "open")
                TownManager.buildingPerk("bank");
            if (this.type === "bank" && this.subtype === "level")
                BankManager.addLevel();
            if (this.type === "cauldron" && this.subtype === "open")
                TownManager.buildingPerk("fusion");
            if (this.type === "cauldron" && this.subtype === "level")
                FusionManager.addLevel();
            if (this.type === "forge" && this.subtype === "open")
                TownManager.buildingPerk("forge");
            if (this.type === "forge" && this.subtype === "level")
                bloopSmith.addLevel();
            if (this.type === "fortune" && this.subtype === "open")
                TownManager.buildingPerk("fortune");
            if (this.type === "fortune" && this.subtype === "level")
                FortuneManager.addLevel();
            if (this.type === "tinker" && this.subtype === "open")
                TownManager.buildingPerk("tinker");
            if (this.type === "tinker" && this.subtype === "level")
                TinkerManager.addLevel();
            if (this.type === "museum" && this.subtype === "open")
                TownManager.buildingPerk("museum");
            if (this.type === "milestone" && this.subtype === "open")
                TownManager.buildingPerk("milestone")
        }
    }, {
        key: "createSave",
        value: function createSave() {
...
function refreshShop() {
    ...
    shopDivs.forEach(function(type) {
        ...
        if (perks.canPurchase === undefined)
            return;
...
function createALperk(perk, name) {
    ...
    if (perkCount > 1)
        $("<div/>").addClass("alPerkCount tooltip").attr({
    ...
    if (!perk.canBuy())
        d5.addClass("cannotAfford");
    else
        d5.removeClass("cannotAfford");
...
var Museum = {
    ...
    donate: function donate(containerID) {
        if (!Inventory.hasContainer(containerID))
            return;
        var container = Inventory.containerToItem(containerID);
        Inventory.removeContainerFromInventory(containerID);
...
var MuseumReward = function() {
    ...
    _createClass(MuseumReward, [{
        ...
        key: "purchase",
        value: function purchase() {
            if (Museum.remainingPoints() < this.purchaseCost())
                return;
            this.lvl += 1
        }
    }, {
...
function showMuseumType(type, skipAnimation) {
    ...
    recipeList.filterByType(type).forEach(function(recipe) {
        var d = $("<div/>").addClass("museumRecipeDiv").appendTo(museumContributionsList);
        if (skipAnimation)
        ...
        recipe.museum.forEach(function(rarity, j) {
            ...
            rarity.forEach(function(sharp, i) {
                ...
                if (sharp)
                    d2.addClass("museumRecipeEntryComplete").html(miscIcons.checkmark);
                else
                    d2.html("+".concat(i))
            })
        })
...
$(document).on("click", ".museumDonate", function(e) {
    ...
    if (Museum.view === "main")
        refreshMuseumTop();
    else if (Museum.view === "rewards")
        showMuseumRewards();
    else
        showMuseumType(Museum.view);
    refreshMuseumInv()
});
...
var Quest = function() {
    ...
    _createClass(Quest, [{
        ...
        key: "addTime",
        value: function addTime(ms) {
            if (this.state !== QuestState.running)
                return;
            this.elapsed += ms;
        ...
        value: function collect() {
            if (this.state === QuestState.success) {
                ...
                if (this.rewardType === "Gold")
                    ResourceManager.addMaterial("M001", this.rewardAmt);
                if (this.rewardType === "Playbook")
                    PlaybookManager.unlockPlaybook(this.rewardAmt)
            }
            this.state = QuestState.idle;
            this.heroids.forEach(function(heroid) {
                var hero = HeroManager.idToHero(heroid);
        ...
        value: function available() {
            if (this.openReqType === "Perk" && !Shop.alreadyPurchased(this.openReq))
                return false;
            if (this.openReqType === "Quest" && !QuestManager.idToQuest(this.openReq).complete)
                return false;
            if (this.openReqType === "Boss" && !DungeonManager.beaten(this.openReq))
                return false;
            if (this.complete && !this.repeatable)
                return false;
            return true
        }
    }]);
...
var QuestManager = {
    ...
    addTime: function addTime(ms) {
        ...
        if (lastTab === "questsTab")
            refreshQuestTimes();
        if (QuestManager.quests.some(function(q) {
            return q.state === QuestState.success && !q.complete || q.state === QuestState.failure && !q.complete
        })) {
            if (this.questblip)
                return;
    ...
    removeParty: function removeParty(heroID) {
        if (heroID === this.hero1)
            this.hero1 = null;
        if (heroID === this.hero2)
            this.hero2 = null;
        if (heroID === this.hero3)
            this.hero3 = null;
        if (heroID === this.hero4)
            this.hero4 = null
    },
    addParty: function addParty(heroID) {
        var quest = this.idToQuest(this.questView);
        var hero = HeroManager.idToHero(heroID);
        if (!this.hero1 && quest.hero1 === heroID)
            return this.hero1 = heroID;
        if (!this.hero2 && quest.hero2 === heroID)
            return this.hero2 = heroID;
        if (!this.hero3 && quest.hero3 === heroID)
            return this.hero3 = heroID;
        if (!this.hero4 && quest.hero4 === heroID)
            return this.hero4 = heroID;
        if (!this.hero1 && quest.hero1 === hero.type)
            return this.hero1 = heroID;
        if (!this.hero2 && quest.hero2 === hero.type)
            return this.hero2 = heroID;
        if (!this.hero3 && quest.hero3 === hero.type)
            return this.hero3 = heroID;
        if (!this.hero4 && quest.hero4 === hero.type)
            return this.hero4 = heroID;
        if (!this.hero1 && quest.hero1 === "Any")
            return this.hero1 = heroID;
        if (!this.hero2 && quest.hero2 === "Any")
            return this.hero2 = heroID;
        if (!this.hero3 && quest.hero3 === "Any")
            return this.hero3 = heroID;
        if (!this.hero4 && quest.hero4 === "Any")
            return this.hero4 = heroID
    },
    pow: function pow() {
    ...
    validTeam: function validTeam() {
        var quest = this.idToQuest(this.questView);
        if (quest.hero1 !== "None" && this.hero1 === null)
            return false;
        if (quest.hero2 !== "None" && this.hero2 === null)
            return false;
        if (quest.hero3 !== "None" && this.hero3 === null)
            return false;
        if (quest.hero4 !== "None" && this.hero4 === null)
            return false;
        return true
    },
...
function refreshQuestText(quest) {
    if (quest.state === QuestState.idle)
        $("#qst" + quest.id).html(displayText("quests_status_idle"));
    if (quest.state === QuestState.running)
        $("#qst" + quest.id).html(msToTime(quest.remaining()));
...
function createQuestContainer(quest, skipAnim) {
    ...
    if (quest.state === QuestState.running)
        d.addClass("questActive");
    if (quest.state === QuestState.success)
        d.removeClass("questActive").addClass("questSuccess");
    if (quest.state === QuestState.failure)
        d.removeClass("questActive").addClass("questFailure");
...
function showQuestParty(skipAnimation) {
    ...
    if (skipAnimation)
        $qpHeader.addClass("questHeaderAnimDisabled");
    ...
    if (!QuestManager.validTeam())
        b1.addClass("qpHeaderInvalidTeam");
...
function setDialogOpen(props) {
    ...
    if (props.id === "patch_notes")
        $(dialogBoxContainer).addClass("isPatchDialog");
    var dialogClose = $("<div/>").attr({
        role: "button",
        tabindex: 1,
        "aria-label": "Close Dialog"
    ...
    if (props.description)
        $("<div/>").addClass("dialogDescription").html(props.description).appendTo(dialogContentContainer);
...
$(document).on("click", ".isDialog", function(e) {
    var id = $(e.currentTarget).attr("data-dialog-id");
    if (settings.dialogStatus === 0 && id)
        setDialogOpen(DialogManager.findDialog(id))
});
...
SkillManager.skillEffects["S0020"] = function(combatParams) {
    var targets = combatParams.getTarget(TargetType.SELF, SideType.ALLIES);
    targets.forEach(function(target) {
        if (target.getBuffStacks("B0020") === 5)
            return;
...
SkillManager.skillEffects["S0022"] = function(combatParams) {
    var targets = combatParams.getTarget(TargetType.SELF, SideType.ALLIES);
    targets.forEach(function(target) {
        if (target.getBuffStacks("B0022") === 5)
            return;
...
SkillManager.skillEffects["S1030"] = function(combatParams) {
    ...
    if (lifeDrain === null || lifeDrain[0].race === "undead")
        return;
...
SkillManager.skillEffects["S2032"] = function(combatParams) {
    ...
    targets.forEach(function(target) {
        var ogPow = combatParams.power;
        if (combatParams.attacker.hp % 10 === 7)
            combatParams.power = Math.floor(combatParams.power * combatParams.attack.mod1);
        target.takeAttack(combatParams);
        combatParams.power = ogPow
    })
...
SkillManager.skillEffects["SM103"] = function(combatParams) {
    var targets = combatParams.getTarget(TargetType.FIRST, SideType.ENEMIES);
    var selfTarget = combatParams.getTarget(TargetType.SELF, SideType.ALLIES)[0];
    if (selfTarget.hp === selfTarget.maxHP())
        combatParams.power = Math.floor(combatParams.power * combatParams.attack.mod1);
    targets.forEach(function(target) {
        target.takeAttack(combatParams)
    })
...
SkillManager.skillEffects["SM107"] = function(combatParams) {
    if (combatParams.attacker.hpLessThan(combatParams.attack.mod1))
        combatParams.power = Math.floor(combatParams.power * combatParams.attack.mod2);
    var targets = combatParams.getTarget(TargetType.FIRST, SideType.ENEMIES);
    targets.forEach(function(target) {
        target.takeAttack(combatParams)
...
SkillManager.skillEffects["SM109"] = function(combatParams) {
    ...
    targets.forEach(function(target) {
        var ogPow = combatParams.power;
        if (target.hpLessThan(combatParams.attack.mod1))
            combatParams.power = Math.floor(combatParams.power * combatParams.attack.mod2);
        target.takeAttack(combatParams);
        combatParams.power = ogPow
    })
...
SkillManager.skillEffects["SM202"] = function(combatParams) {
    ...
    targets.forEach(function(target) {
        if (target.hpLessThan(combatParams.attack.mod1))
            combatParams.power = Math.floor(originalDmg * combatParams.attack.mod2);
        else
            combatParams.power = originalDmg;
        target.takeAttack(combatParams)
    })
}
;
...
SkillManager.skillEffects["SM905A"] = function(combatParams) {
    ...
    if (this.state === undefined || this.state === targets.length - 1)
        this.state = 0;
    else
        this.state += 1;
...
SkillManager.skillEffects["SMP904A"] = function(type, target, attack, skillParams) {
    if (type !== "dead" || target.state !== null)
        return;
...
SkillManager.skillEffects["SMP906"] = function(type, target, attack, skillParams) {
    if (type !== "onTurn")
        return;
    if (target.hp > target.maxHP() / 4 || target.state !== null)
        return;
    target.state = "egg";
...
SkillManager.skillEffects["SMP907"] = function(type, target, attack, skillParams) {
    ...
    if (type === "onHit") {
        ...
        if (!target.hpLessThan(.25 * stacks))
            return;
